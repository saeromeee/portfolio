...
%>
	        ...
	        <tr align="center" >
	          <th scope="col" onclick="location.href='View.jsp?num=<%= dto.getNum() %>';"><%= virtualNum %></th>  <!--게시물 번호-->
	            <td align="left" onclick="location.href='View.jsp?num=<%= dto.getNum() %>';">  <!--제목(+ 하이퍼링크)-->
	                <a href="View.jsp?num=<%= dto.getNum() %>" style="color:black;"><%= dto.getTitle() %></a> 
	            </td>
	            <td align="center" onclick="location.href='View.jsp?num=<%= dto.getNum() %>';"><%= dto.getId() %></td>          <!--작성자 아이디-->
	            <td align="center" onclick="location.href='View.jsp?num=<%= dto.getNum() %>';"><%= dto.getVisitcount() %></td>  <!--조회수-->
	            <td align="center" >
	            <%= dto.getPostdate() %>
	            <button type="button" class="btn btn-secondary" 
	            onclick="location.href='remove.jsp?remove=1&num=<%=dto.getNum() %>';" 
	            style="margin-left:2%; z-index:99; width:50px; height:23px; font-size:9px; ">삭제</button>
	            </td>    <!--작성일-->
	        </tr>
...
<!--     		<button type="button" class="btn btn-secondary" onclick="location.href='remove.jsp?remove=member';">멤버리셋</button> -->
        ...
        <div align="right" style="width:30%; margin-right:7%;">          	
            ...
            <button type="button" class="btn btn-secondary" onclick="location.href='List.jsp';">복귀 </button>
        </div> 
    </div> 
</body>
</html>
